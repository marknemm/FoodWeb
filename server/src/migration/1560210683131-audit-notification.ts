...
export class auditNotification1560210683131 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    await queryRunner.query(`CREATE TYPE "audit_eventtype_enum" AS ENUM('Signup', 'Verify Account', 'Remove Unverified Account', 'Update Account', 'Request Password Reset', 'Reset Password', 'Donate', 'Update Donation', 'Remove Donation', 'Claim Donation', 'Unclaim Donation', 'Schedule Delivery', 'Pickup Delivery', 'Complete Delivery')`);
    await queryRunner.query(`CREATE TABLE "Audit" ("id" SERIAL NOT NULL, "eventType" "audit_eventtype_enum" NOT NULL, "data" json NOT NULL, "recaptchaScore" integer, "timestamp" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "PK_fc6e886590a36b630dd98096fc3" PRIMARY KEY ("id"))`);
