...
export class directions1576463243834 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    let results: any[] = await queryRunner.query(`
      SELECT      "delivery"."id"                                   AS "deliveryId",
                  ...
                  ST_X("volunteerContactInfo"."location"::geometry) AS "volunteerLon"
    ...
    for (const result of results) {
      const origin: LatLngLiteral = { lat: result.donorLat, lng: result.donorLon};
      const destination: LatLngLiteral = { lat: result.volunteerLat, lng: result.volunteerLon };
      const directions: Directions = await getDirections([origin, destination]);
      await queryRunner.query(`
        UPDATE  "Delivery"
    ...
    for (const result of results) {
      const origin: LatLngLiteral = { lat: result.donorLat, lng: result.donorLon };
      const destination: LatLngLiteral = { lat: result.receiverLat, lng: result.receiverLon };
      const directions: Directions = await getDirections([origin, destination]);
      await queryRunner.query(`
