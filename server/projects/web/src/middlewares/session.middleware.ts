...
export function ensureSessionAdmin(request: Request, response: Response, next: NextFunction): void {
  if (request.session.account && process.env.adminUsernames.indexOf(request.session.account.username) >= 0) {
    next(); // Call the next route handler.
  } else {
    response.status(302).send({ message: 'Authentication required' });
  }
}
