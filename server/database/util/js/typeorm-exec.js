...
async function execOrmCli() {
  ...
  if (await isPortReachable(5432)) { // Try local.
    execSync(`ts-node -r tsconfig-paths/register ${typeOrmCliPath} -f ormconfig.json ${typeOrmArgs}`);
  } else if(await isPortReachable(5432, { host: 'postgres' })) { // Try docker.
    execDockerOrmCli(typeOrmCliPath, typeOrmArgs);
  } else { // Give up.
 ...
 */
function execDockerOrmCli(typeOrmCliPath, typeOrmArgs) {
  const dockerOrmConfigPathname = createDockerOrmConfig();
  updateDockerOrmConfigHost(dockerOrmConfigPathname);
  execSync(`ts-node -r tsconfig-paths/register ${typeOrmCliPath} -f docker-ormconfig.json ${typeOrmArgs}`);
...
function updateDockerOrmConfigHost(dockerOrmConfigPathname) {
  ...
  fs.writeFileSync(dockerOrmConfigPathname, JSON.stringify(dockerOrmConfigJSON), { encoding: 'utf8', flag: 'w' });
}
