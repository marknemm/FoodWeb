...
export class HybridAuthenticationService extends AuthenticationService {
   ...
   */
   private get _appSessionToken(): string {
    return this._sessionService.get('appSessionToken');
  }

  private set _appSessionToken(token: string) {
    (token)
      ? this._sessionService.save('appSessionToken', token)
      : this._sessionService.delete('appSessionToken');
  }
